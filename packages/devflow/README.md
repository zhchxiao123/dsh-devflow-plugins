...
| `attachArtifact(request)` | Registers a stage deliverable in the journal against the current stage; rejected while `blocked` or `done`, with `revision-mismatch`, `illegal-edge`, or `write-contended`. |
