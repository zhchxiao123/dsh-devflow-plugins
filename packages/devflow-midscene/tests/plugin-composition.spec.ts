...
describe('Midscene acceptance in a real Loader composition', () => {
  it('serves the invocable acceptance runbook and withdraws it without destroying the registry', async () => {
    ...
    expect(skill?.content).toContain('devflow_attach_artifact')
    expect(skill?.content).toContain('do not register artifacts inside a transition gate')
    expect(skill?.content).toContain('job_kill')
