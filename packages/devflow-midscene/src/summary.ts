...
export async function recordPreflight(p: AcceptanceProfile, profile: string, model: 'available' | 'unknown' | 'unavailable'): Promise<void> {
  ...
  await writeFile(temp, JSON.stringify({ workspace, profile, model, modelId: p.model, family: p.family, at: new Date().toISOString() }), { flag: 'wx', mode: 0o600 })
  ...
  ctx.effect(() => ctx.provide('devflowMidsceneSummary', {
    async read(sessionId: string, cardId: string): Promise<MidsceneSummary> {
      ...
      for (const [name, p] of Object.entries(config.profiles)) {
        if (await realpath(p.workspace) === workspace) profiles.push(await profileSummary(p, name, workspace, sessionId, cardId))
      }
