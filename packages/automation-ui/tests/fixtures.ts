...
export function subscription(id = 'sub'): Subscription {
  return { id, repository: `owner/${id}`, actor: 'test', issues: true, discussions: false, revision: 1, paused: false }
}
export function plan(id = 'plan'): Plan {
  return { id, name: `Schedule ${id}`, handler: 'github.sync', params: { subscriptionId: 'sub' }, rule: { kind: 'interval', everyMs: 120_000 }, enabled: true, deleted: false, nextAt: 1000, createdBy: 'test', updatedBy: 'test' }
}
...
export function overview(): Overview {
  return { schedulerAvailable: true, githubAvailable: true, plans: [plan()],
