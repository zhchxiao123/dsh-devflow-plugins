...
Any service failing to start rolls every already-started service back in reverse order before `env_up` returns; the failed service's report carries the phase, the exit facts, and its log tail. A running environment is registered as an effect of the plugin's fiber whose disposer is the whole teardown, so the session ending tears the environment down — orphaned service processes are ruled out structurally, not by cleanup code. The workspace root is the process cwd captured when the plugin applies (the same assumption `dsh-devflow-filesystem`'s default root rests on); the manifest path and every service `cwd` resolve against it.

## The tools

| tool | arguments | wire value |
|---|---|---|
| `env_up` | — | `{ ok, services: [{ name, state: ready\|failed\|not-started, detail?, logTail? }], teardownDetail? }`; `teardownDetail` reports the rollback's own residue when tearing the started services back down itself failed. |
| `env_status` | — | Same shape, re-probed: every readiness probe runs again, so the answer is current health, not whether `env_up` once succeeded. No services while the environment is not up. |
...
| `integration_test` | — | `{ passed, phase: up\|seed\|test, exitCode?, outputTail?, detail?, services? }` — brings the environment up when it is not, runs `seed` when declared, then `test`; the report names the phase that settled it. The environment stays up afterwards for re-runs. |
