 ...
 * A missing or invalid manifest surfaces every field-path issue plus the
 * pointer to the `testenv-bootstrap` skill, which owns writing and repairing
 * `testenv.yml`.
 */
...
import type { ServiceStartReport } from './types.ts'
...
/** The env_up wire value: the environment verdict plus the rollback's own residue. */
const ENV_UP_OUTPUT = {
  ...ENVIRONMENT_OUTPUT,
  properties: {
...
/** One projected service entry; the render helper reads exactly these fields. */
...
/** Model-facing lines, one block per service: state, name, then failure facts. */
...
export function registerTools(ctx: Context, engine: TestenvEngine): void {
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'readiness probe. Returns one entry per service; a failed service carries its log tail, and '
      + 'any services already started are torn back down. Use it to bring the environment up before '
      + 'working against live services; env_status re-checks health later, env_down tears it down, '
      + 'and integration_test brings the environment up by itself. If there is no valid testenv.yml '
      + 'yet, run the testenv-bootstrap skill to research and write one.',
    parameters: {},
    output: {
      ...
      render: (_args, value) => [{
        ...
        text: [
          value.ok
            ? 'Environment is up; every service is ready.'
            : value.teardownDetail === undefined
              ? 'Environment failed to start; every started service was torn back down.'
              : 'Environment failed to start, and rolling the started services back left residue.',
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'merely whether env_up once succeeded. Reports no services while the environment is not up. '
      + 'Use it before pointing tests at a long-lived environment, or to find which service decayed; '
      + 'read a decayed service\'s output with env_logs.',
    parameters: {},
    output: {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'code and a bounded output tail; a failed up phase reports per-service startup state '
      + 'instead. The environment stays up afterwards for re-runs; env_down tears it down. If there '
      + 'is no valid testenv.yml yet, run the testenv-bootstrap skill first.',
    parameters: {},
    output: {
      schema: {
        type: 'object',
        ...
        properties: {
          ...
          services: {
            ...
            description: 'Per-service startup report of a failed up phase.',
          },
        },
      },
      render: (_args, value) => {
        if (value.phase === 'up') {
          return [{
            type: 'text',
            text: [
              'Integration test failed: the environment did not start.',
              ...serviceLines(value.services ?? []),
            ].join('\n'),
          }]
        }
        ...
        return [{
          ...
          text: [
            value.passed
              ? `Integration test passed${exit}.`
              : `Integration test failed during the ${value.phase} phase${exit}.`,
    ...
    async execute() {
      ...
      if (report.phase === 'up') {
        return { passed: false, phase: 'up' as const, services: report.up.services.map(projectService) }
      }
      return {
