...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    execute: async () => ({ subscriptions: await ctx.githubSync.subscriptions(), storage: await ctx.githubSync.storage() }),
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      const spec = {
        repository: args.repository, issues: args.issues, discussions: args.discussions, actor: actor(exec),
      ...
      return subscriptionId === undefined
        ...
        : ctx.githubSync.updateSubscription(subscriptionId, spec)
    },
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => ctx.githubSync.updateSubscription(args.subscriptionId, { paused: args.action === 'pause', actor: actor(exec) }),
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args) => {
      if (args.runId === undefined) return ctx.githubSync.runs(args.subscriptionId)
      if (args.subscriptionId !== undefined) throw new Error('Choose runId or subscriptionId, not both')
      const value = await ctx.githubSync.run(args.runId)
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => { await ctx.githubSync.cancel(args.runId, actor(exec)); return { runId: args.runId } },
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => ctx.githubSync.resumeRun(args.runId, actor(exec)),
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async args => ctx.githubSync.snapshots(args.subscriptionId), isConcurrencySafe: () => true,
    presentCall: args => ({ card: 'generic', kind: 'read', title: '读取 GitHub 同步内容', rawInput: args.subscriptionId }), presentResult,
  }))
  ctx.tools.register(defineTool({
    name: 'github_sync_capacity', description: 'Explicitly set the local intake storage capacity in positive bytes. Does not delete existing content. Increasing capacity clears admission blocking when sufficient; resume the original failed/partial run explicitly.', parameters: { bytes: integer },
    output: { schema: storage, render: (_args, value) => [{ type: 'text', text: `Storage: ${value.bytes}/${value.capacityBytes} bytes; blocked=${value.blocked}.\n${JSON.stringify(value, null, 2)}` }] },
    execute: async (args, exec) => ctx.githubSync.setCapacity(args.bytes, actor(exec)),
    presentCall: args => ({ card: 'generic', kind: 'edit', title: '调整 GitHub 同步容量', rawInput: args.bytes }), presentResult,
  }))
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      actor(exec)
      if (args.action === 'register') await ctx.githubSync.registerConsumer(args.subscriptionId, args.consumerId, args.from)
      else await ctx.githubSync.replay(args.subscriptionId, args.consumerId, args.from)
      return ctx.githubSync.consumerState(args.subscriptionId, args.consumerId)
    },
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      actor(exec)
      return {
        changes: await ctx.githubSync.readChanges(args.subscriptionId, args.consumerId, args.limit),
        cursor: await ctx.githubSync.consumerState(args.subscriptionId, args.consumerId),
      }
    },
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      actor(exec)
      await ctx.githubSync.acknowledge(args.subscriptionId, args.consumerId, args.sequence)
      return ctx.githubSync.consumerState(args.subscriptionId, args.consumerId)
    },
  ...
  ctx.tools.register(defineTool({
    ...
    output: { schema: cursor, render: (_args, value) => [{ type: 'text', text: `Consumer cursor: acknowledged=${value.acknowledged}, delivered=${value.delivered}.` }] },
    execute: async args => ctx.githubSync.consumerState(args.subscriptionId, args.consumerId), isConcurrencySafe: () => true,
