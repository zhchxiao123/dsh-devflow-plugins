...
The toolbar reports total, in-progress, blocked, and completed counts, and selects which cards the board is about: work in progress, or the archive. Scope is a separate axis from the Kanban/list switch, so the archive is reachable from either — a reader who just filed their last card asks "where did it go", not "show me extra rows", and a control buried in one view answers neither. The toolbar is chrome: it stays while the active set is empty, which is exactly when the route to the archive matters most. Archived cards read as a read-only group, each tagged by how it left, because only a filed card can come back and that is a `/devflow` decision. Entering the scope is what fetches the archive, and leaving discards it: the set is unbounded and secondary to the board. Its pages accumulate behind a "load more" control, and a change frame restarts them rather than resuming, because a card filed or restored between two pages shifts the set the cursor was issued against. The scope holds no stage columns — filed work does not flow, so the view switch is disabled there with its reason shown rather than quietly removed.
