...
工具栏显示总数、进行中、受阻和已完成数量。加载中、确定为空、首次读取失败是三种不同状态；失败可以重试，已有成功数据时后台刷新失败会保留上一版看板。打开卡片后可以查看需求、拆分关系、阶段产物、持有者、命名流水线，以及按新到旧排列的 journal 时间线，各区块默认展开并可折叠。官方侧栏处于普通形态时，详情替换看板并提供返回操作；进入官方全屏形态后，看板与详情并排显示。
