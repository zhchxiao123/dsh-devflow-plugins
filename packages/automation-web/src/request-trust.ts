/**
 * Browser-trust fence for automation's own route, restating the rule the harness
...
export function assertTrustedAuthority(entry: string): void {
  ...
  throw new Error(`automation-web: trustedHosts entry ${JSON.stringify(entry)} is not a bare host[:port] authority`)
}
