...
**完整输出写进部署指定的目录,而不是写到卡片上。** 这一部分没有按计划走。显而易见的设计是用 `attachArtifact` 登记输出,毕竟 store 已经有这个概念,而卡片正是失败该归属的地方。**它会死锁**:store 按卡片串行化,而这道波布跑在**正持有该卡片这一轮**的 transition 内部,于是这次调用会去等一个正在等它的 transition。本包里的 `parkBlocked` 一直靠不 await 自己的 `transition` 调用躲开了这一点 —— 那里的注释把它称作"把 blocked 停放移动排在被否决的 transition 的串行化之后",读起来像是在讲顺序,而它同时也是唯一让那里不挂死的东西。

直接写进卡片目录是另一个候选,因分层被否决:卡片的磁盘形态属于 provider,一个拼出 `root/tasks/<id>/artifacts` 的策略插件,等于把一份它并不拥有的布局写死了。

所以 `failureLogDir` 是一个普通路径,默认不设。想要完整输出的部署自己指定一个地方放;不想要的保持今天的行为,不会凭空多出没要过的东西。写日志失败只告警,否决照常成立 —— 门禁的职责是判定,不是保证日志可靠。
